...
class Project {
  ...
  getSlug({ projectOwner, projectName, projectVersion }) {
    return `${projectOwner}/${projectName}:${projectVersion}`
  }

  getSlugFromDeployment({ owner_name, repo_name, tag }) {
    return `${owner_name}/${repo_name}:${tag}`
  }
}
